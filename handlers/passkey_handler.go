...
func (h *WebAuthnHandler) WebAuthnRegistrationEndHandler(w http.ResponseWriter, r *http.Request) {
	...
	session, okSession := h.storage.GetSession(sid.Value)
	h.logger.Info(string(session.UserID))
	if !okSession {
		h.logger.Error("Failed to find session data", err)
		http.Error(w, "Invalid request session", http.StatusBadRequest)
		return
	}
